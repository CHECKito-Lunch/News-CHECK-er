...
// optional, aber nützlich falls du Revalidierung/Edge verhinderst
...
export async function POST(req: Request) {
  ...
  const poll: PollRow = {
    ...
    question: (body.question ?? 'Deine Frage?').toString(),
    options: Array.isArray(body.options) && body.options.length
      ? body.options.map(String)
      : ['Option A', 'Option B'],
    multi_choice: !!body.multi_choice,
    max_choices: Number.isFinite(body.max_choices as any) ? Number(body.max_choices) : 1,
  ...
  // Wenn single-choice, safety: max_choices = 1
