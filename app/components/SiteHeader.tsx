...
const nav = [
  ...
  // Admin-Link nur zeigen, wenn Rolle passt (s.u.)
];
...
export default function SiteHeader() {
  ...
  useEffect(() => { refetchMe(); }, [pathname]); // bei Routewechsel neu holen
  useEffect(() => {
    const h = () => refetchMe();
  ...
  const canSeeAdmin = me && (me.role === 'admin' || me.role === 'moderator');
        ...
        <nav className="hidden sm:flex items-center gap-1">
          ...
          {canSeeAdmin && (
            <Link
              href="/admin"
              className={`px-3 py-2 rounded-lg text-sm font-medium ${
                pathname?.startsWith('/admin') ? 'bg-blue-600 text-white'
                : 'text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800'
              }`}
            >
              Admin
          ...
          {me && (
            <Link
              href="/profile"
              className={`px-3 py-2 rounded-lg text-sm font-medium ${
                pathname?.startsWith('/profile') ? 'bg-blue-600 text-white'
                : 'text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800'
              }`}
            >
              Profil
