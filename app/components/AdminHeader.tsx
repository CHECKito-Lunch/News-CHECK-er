...
export default function AdminHeader({ initialRole }: { initialRole?: Role }) {
  ...
  // Unread laden wie SiteHeader (mit Breakdown)
  useEffect(() => {
  ...
  // Links wie im SiteHeader (+ Admin-Kachel)
  const links = useMemo(() => {
    const arr: { href: string; label: string; icon: JSX.Element }[] = [
      ...
      { href: '/feedback',  label: 'Kunden Feedbacks', icon: <IconChat/> },
      { href: '/quality',   label: 'Mitarbeiter Feedbacks', icon: <IconClipboardCheck/> },
    ];
    if (me && me.role === 'teamleiter') {
      arr.push({ href: '/teamhub', label: 'Teamhub', icon: <IconTeam/> });
    }
    if (me) {
      arr.push({ href: '/profile', label: 'Profil', icon: <IconUser/> });
    }
    if (me && (me.role === 'admin' || me.role === 'moderator' || me.role === 'teamleiter')) {
      arr.push({ href: '/admin', label: 'Adminbereich', icon: <IconShield/> });
    }
    return arr;
  }, [me]);
...
/>
      ...
      {/* Ausklapp-Menü im SiteHeader-Stil */}
